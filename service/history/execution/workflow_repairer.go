...
func (r *workflowRepairerImpl) terminateWithHistoryEvent(
	...
	// Note: CloseExecutionTask and DeleteHistoryEventTask are already generated by
	// AddWorkflowExecutionTerminatedEvent → ReplicateWorkflowExecutionTerminatedEvent →
	// GenerateWorkflowCloseTasks, so they are included in the mutation regardless of policy.
...
func (r *workflowRepairerImpl) addRetentionTasksToMutation(
	...
	const defaultRetentionDays int32 = 1
	retentionDays := defaultRetentionDays
	if domainEntry, err := r.shard.GetDomainCache().GetDomainByID(executionInfo.DomainID); err == nil {
		retentionDays = int32(domainEntry.GetRetentionDays(executionInfo.WorkflowID))
	}
	...
	if mutation.TasksByCategory == nil {
		mutation.TasksByCategory = make(map[persistence.HistoryTaskCategory][]persistence.Task)
	}
